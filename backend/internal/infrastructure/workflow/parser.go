...
func ParseWorkflowFull(data []byte) (*WorkflowIR, []Diagnostic, error) {
	...
	if doc.Kind != yaml.MappingNode {
		diags = append(diags, Diagnostic{Line: doc.Line, Col: doc.Column, Severity: "error", Message: "workflow root must be a mapping"})
		return nil, diags, nil
	}
	...
	if onNode == nil {
		diags = append(diags, Diagnostic{Line: doc.Line, Col: doc.Column, Severity: "error", Message: "workflow must define trigger 'on'"})
	}
	if jobsNode == nil {
		diags = append(diags, Diagnostic{Line: doc.Line, Col: doc.Column, Severity: "error", Message: "workflow must contain at least one job"})
		return nil, diags, nil
	}
	...
	if err := jobsNode.Decode(&rawJobs); err != nil {
		diags = append(diags, Diagnostic{Line: jobsNode.Line, Col: jobsNode.Column, Severity: "error", Message: err.Error()})
		return nil, diags, nil
	}
	if len(rawJobs) == 0 {
		diags = append(diags, Diagnostic{Line: jobsNode.Line, Col: jobsNode.Column, Severity: "error", Message: "workflow must contain at least one job"})
		return nil, diags, nil
	}
...
func expandMatrix(node yaml.Node) ([]map[string]any, []Diagnostic) {
	...
	if err := node.Decode(&raw); err != nil {
		diags = append(diags, Diagnostic{Line: node.Line, Col: node.Column, Severity: "error", Message: err.Error()})
		return nil, diags
	}
