...
func (m *mockSequencer) NextNumber(context.Context, string) (int64, error) {
	m.next++
	return m.next, nil
}
...
func TestDuplicateName(t *testing.T) {
	...
	uc := repository.NewCreateRepositoryUsecase(repos, &mockSequencer{})
...
func TestValidCreate(t *testing.T) {
	repos := &mockRepositoryRepo{byOwnerAndName: map[string]*domain.Repository{}}
	seq := &mockSequencer{}
	uc := repository.NewCreateRepositoryUsecase(repos, seq)
