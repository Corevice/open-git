...
func (h *RepositoryHandler) Register(g *echo.Group, authMiddleware echo.MiddlewareFunc) {
	g.POST("/user/repos", h.Create, authMiddleware, middleware.RequireScope("repo"))
	g.GET("/repos/:owner/:repo", h.Get, middleware.OptionalAuth())
	g.PATCH("/repos/:owner/:repo", h.UpdateVisibility, authMiddleware, middleware.RequireScope("repo"))
	g.DELETE("/repos/:owner/:repo", h.Delete, authMiddleware, middleware.RequireScope("repo"))
}
