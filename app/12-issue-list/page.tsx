...
export default function IssueListPage() {
  ...
  const [selectedId, setSelectedId] = useState<number>(1247);
    ...
    <div className="min-h-screen bg-[#f6f8fa] text-[#1f2328]">
      ...
      <header className="h-16 bg-white/85 backdrop-blur border-b border-[#d0d7de] flex items-center justify-between px-6 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          <input
            className="w-72 px-3 py-1.5 border border-[#d0d7de] rounded-md bg-white text-sm"
            placeholder="Search or jump to..."
            value={headerSearch}
            onChange={(e) => setHeaderSearch(e.target.value)}
          />
          <Link href="/13-pr-list" className="text-sm px-2 py-1 hover:text-[#0969da]">
            Pull Requests
          </Link>
          <Link href="/12-issue-list" className="text-sm px-2 py-1 hover:text-[#0969da]">
            Issues
          </Link>
      ...
      <nav className="bg-white border-b border-[#d0d7de] px-6 flex gap-1">
        ...
        <Link
          href="/12-issue-list"
          className="px-4 py-3 text-sm border-b-2 border-[#fd8c73] font-semibold"
        >
          ⊙ Issues <span className="ml-1 text-xs px-1.5 py-0.5 rounded-full bg-[#ddf4ff] text-[#0969da]">42</span>
        </Link>
        <Link href="/13-pr-list" className="px-4 py-3 text-sm border-b-2 border-transparent hover:border-[#d0d7de]">
          ⇄ Pull requests <span className="ml-1 text-xs px-1.5 py-0.5 rounded-full bg-[#eaeef2]">7</span>
        </Link>
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="py-4 text-xl">
          ...
          <span className="ml-2 text-xs px-2 py-0.5 rounded-full border border-[#d0d7de] text-[#656d76] font-normal">
            Public
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-5 py-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <div className="flex gap-2">
                <button className="px-3 py-1.5 text-sm border border-[#d0d7de] rounded-md bg-white hover:bg-[#f6f8fa]">
                  Labels <span className="text-[#656d76]">18</span>
                </button>
                <button className="px-3 py-1.5 text-sm border border-[#d0d7de] rounded-md bg-white hover:bg-[#f6f8fa]">
                  Milestones <span className="text-[#656d76]">4</span>
                </button>
              </div>
              <Link
                href="/12-issue-list"
            ...
            <div className="flex gap-2 items-center bg-[#f6f8fa] border border-[#d0d7de] rounded-t-md px-4 py-3">
              ...
              />
              {["Author", "Label", "Projects", "Milestones", "Assignee", "Sort"].map(
                (k) => (
                  <button
                    key={k}
                    className="px-3 py-1.5 text-sm text-[#1f2328] hover:text-[#0969da]"
                  >
                    {k} ▾
                  </button>
                )
            ...
            <div className="flex gap-2 items-center bg-[#f6f8fa] border-x border-[#d0d7de] px-4 py-3">
              <div className="flex gap-4 text-sm font-semibold">
                <span className="text-[#1f2328]">⊙ 42 Open</span>
                <span className="text-[#656d76] font-normal">✓ 187 Closed</span>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {issue.labels.map((l) => (
                        <span
                          key={l.text}
                          className={`inline-block px-2 ml-1.5 rounded-[10px] text-[11px] font-semibold leading-[18px] text-white ${l.color}`}
                        >
                          {l.text}
                        </span>
                      ))}
          ...
          {/* Detail panel */}
          <aside className="bg-white border border-[#d0d7de] rounded-md p-5 h-fit lg:sticky lg:top-5">
            ...
            <div className="py-3 border-t border-[#d0d7de]">
              <h4 className="text-xs text-[#656d76] font-semibold uppercase mb-2">Assignees</h4>
            ...
            <div className="py-3 border-t border-[#d0d7de]">
              <h4 className="text-xs text-[#656d76] font-semibold uppercase mb-2">Labels</h4>
              <span className="inline-block px-2 mr-1.5 rounded-[10px] text-[11px] font-semibold leading-[18px] text-white bg-[#d1242f]">
                bug
              </span>
              <span className="inline-block px-2 rounded-[10px] text-[11px] font-semibold leading-[18px] text-white bg-[#8250df]">
                help wanted
              </span>
            </div>

            <div className="py-3 border-t border-[#d0d7de]">
              <h4 className="text-xs text-[#656d76] font-semibold uppercase mb-2">Milestone</h4>
            ...
            <div className="py-3 border-t border-[#d0d7de]">
              <h4 className="text-xs text-[#656d76] font-semibold uppercase mb-2">
            ...
            <div className="py-3 border-t border-[#d0d7de]">
              ...
              <div className="py-3 border-t border-[#d0d7de]">
                ...
                <div className="text-sm pl-8">
                  PR #1251 を作成しました。レビューお願いします。
                </div>
              </div>
            </div>

            <form onSubmit={handleCommentSubmit} className="mt-4">
              <div className="flex gap-1 p-1.5 border border-[#d0d7de] border-b-0 rounded-t-md bg-[#f6f8fa] text-sm">
                {["B", "I", "</>", "🔗", "📷"].map((t) => (
              ...
              <div className="flex gap-2 justify-end mt-3">
                <button
                  type="button"
                  className="px-4 py-1.5 text-sm border border-[#d0d7de] rounded-md bg-white"
                >
                  Close issue
                </button>
                <button
                  type="submit"
                  className="px-4 py-1.5 text-sm bg-[#1f883d] text-white rounded-md font-semibold border border-black/10"
                >
                  Comment
                </button>
              </div>
